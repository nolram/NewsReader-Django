...
class NewsHandler(WebSocketHandler):
    """
        Javascript de teste:
        var socket = new WebSocket('ws://127.0.0.1:8080/last_news/1');
    ...
    def open(self, news):
        """
            Registra para armazenar as últimas coletas do banco de dados
        """
        self.news = news
        self.application.add_subscriber(self.news, self)

    def on_message(self, message):
        """
            Faz o broadcast das atualizações para outros clientes interessados.
        """
        self.application.broadcast(message, channel=self.news, sender=self)
    ...
    def on_close(self):
        ...
        :return:
        """
        self.application.remove_subscriber(self.news, self)


class FlynApplication(Application):

    def __init__(self, **kwargs):
        routes = [
            (r'/last_news/(?P<news>[0-9]+)', NewsHandler),
        ]
...
if __name__ == "__main__":
    parse_command_line()
    application = FlynApplication(debug=options.debug)
